...
export default function RegionalSummary({result}:{result:Result}) {
 ...
 return <section className="regional-summary" aria-label="Regional energy comparison">
  ...
  <details className="regional-method"><summary>Compare the 4 energy approaches</summary><div className="approach-list">{summary.comparison?.map(row=><details key={row.id} className={row.eligible?'':'approach-unavailable'}>
